...
5. Write/publish requests: **flag + confirm gates** — if writes are flagged off or Free Google lacks the write scopes, refuse (`WRITE_NOT_ENABLED` / `CONSENT_W_REQUIRED` / `CONSENT_G_REQUIRED` / `CONSENT_S_REQUIRED`) and point at `auth login` (legacy `login-write` / `login-ga4-admin` / `login-gsc-write` alias; they do not flip `DGTL_WRITES_ENABLED`). Tag / trigger / variable writes exist; **publish last**. Do **not** eternally claim “there is no publish tool”; do **not** invent confirm phrases. Marketplace default stays flag **off**. Ads / Meta / TikTok stay Pro.
...
8. **Not all tools are read.** Free Google can hold manage scopes; mutates still need the flag + confirm. Shopify / Klaviyo / GBP (when flag on) stay fail-closed without their own grants. Ads/Meta mutate/create stay Pro. See [TOOLS.md](TOOLS.md) Mutate honesty.
