...
| Recs → approve → push | `skills/recs-approve-push/` | Sequence Ads recs, MC issues, GTM workspace diff, Klaviyo flow status, missing GA4 Ads link. One mutate per confirm. No apply-all. ENABLED is not a side effect. |
...
5. Write/publish requests: **confirm gates** — if Free Google lacks the write scopes, refuse (`CONSENT_W_REQUIRED` / `CONSENT_G_REQUIRED` / `CONSENT_S_REQUIRED`) and point at `auth login` (legacy `login-write` / `login-ga4-admin` / `login-gsc-write` alias). Live mutate needs `dry_run=false` plus a user confirm that includes the resource id. Tag / trigger / variable writes exist; **publish last**. Do **not** eternally claim “there is no publish tool”; do **not** invent confirm phrases. `DGTL_WRITES_ENABLED` is Merchant Center ProductInput only. Ads / Meta / TikTok stay Pro.
...
8. **Not all tools are read.** Free Google can hold manage scopes; live mutates need in-chat confirm (`dry_run` default). Shopify / Klaviyo stay fail-closed without their own grants + confirm. GBP (when flag on) stays fail-closed without Consent B. Ads/Meta mutate/create stay Pro. See [TOOLS.md](TOOLS.md) Mutate honesty.
...
| “Apply these Ads recs / approve and push / send the draft” | `recs-approve-push` |
...
| shopify-readonly | `shopify_get_shop`, `shopify_list_*`, `shopify_get_*` | Calling Admin API without credentials; inventing ids; live writes without confirm; silent scope expand |
| klaviyo-readonly | `klaviyo_get_account`, `klaviyo_list_*`, `klaviyo_get_*` | Calling Klaviyo without a `pk_`; dumping full profile/review PII; sending from draft create; Polar OAuth; inventing `$shopify:::` ids; logging the key |
...
| recs-approve-push | `gads_search` recommendations, `gads_apply_recommendation` / `gads_apply_recommendations`, `mc_list_account_issues` / `mc_list_product_statuses`, `gtm_get_live_container_version` + workspace lists, `klaviyo_list_flows` / `klaviyo_get_flow`, `ga4_list_google_ads_links`, then one of `mc_upsert_product_input` / `gtm_publish_container` / `ga4_create_google_ads_link` / `klaviyo_create_campaign_send_job` | Apply-all recs; ENABLED as apply side effect; two mutates per confirm; send job from draft create |
