...
Closed Consent A kernel: [TOOLS.md](TOOLS.md). Machine copy: `schemas/v1/catalog.json` (`count`: **24**). Shopify is local-free (not Polar, not the 24 kernel). Ads/Meta/Merchant Center are Polar `LICENSE_REQUIRED`. MC hops **direct** to Merchant API (Consent MC) — not stamp. Write/publish stubs (if registered) are gated (`WRITE_NOT_ENABLED` / Consent W) and are **not** Consent A listing promises.
