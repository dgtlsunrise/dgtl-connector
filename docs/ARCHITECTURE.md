...
Closed Consent A kernel: [TOOLS.md](TOOLS.md). Machine copy: `schemas/v1/catalog.json` (`count`: **26**). Wave 13 added `gtm_list_clients` + `gtm_list_environments` (honest bump from 24). Shopify is local-free (not Polar, not the 26 kernel) — reads (including Wave 15 publications/catalogs/feeds) plus confirm-gated `shopify_adjust_inventory` / `shopify_product_set` hop **direct** Admin GraphQL (`direct_shopify`). Stamp has **zero** `shopify_*` names; multi-store vault is out of Wave 7. Explicit Shopify scope expand is documented — never silent. Ads/Meta/Merchant Center are Polar `LICENSE_REQUIRED`. MC hops **direct** to Merchant API (Consent MC) — not stamp. GBP hops **direct** to GBP APIs (Consent B, `DGTL_GBP_ENABLED`) — not stamp, not Consent A. Consent W GTM writes (tag / trigger / variable / publish / client / container / environment) hop **direct** via `GoogleWriteHttp` — not stamp. They need Free Google manage scopes plus confirm (`CONSENT_W_REQUIRED` when scopes are missing) and are **not** Consent A listing promises. Consent G / Consent S are **separate write consents** (stores + login CLIs in Wave 10). Wave 11 registers named GA4 Admin tools via `GoogleGa4AdminHttp` (Consent G; Connect + confirm). Wave 12 registers `gsc_submit_sitemap` / `gsc_delete_sitemap` via `GoogleGscWriteHttp` (Consent S; same confirm rule). `DGTL_WRITES_ENABLED` remains for live Merchant Center ProductInput writes only. No Indexing API. No stamp ingest keys in GTM clients.
...
Consent G (`google-oauth-ga4-admin.json`) and Consent S (`google-oauth-gsc-write.json`) are **separate** write-consent stores (mode 0600). They are never the Consent A file. Login CLIs `auth login-ga4-admin` / `auth login-gsc-write` alias to Free Google and do not enable `DGTL_WRITES_ENABLED` (that flag is MC-only). Doctor and `support_packet` report store **existence** only — never tokens.
