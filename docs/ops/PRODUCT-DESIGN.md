...
| **G** — GA4 Admin writes | **Separate** Desktop client (or later extend W — **never A**). Env: `GOOGLE_OAUTH_GA4_ADMIN_CLIENT_ID` / `SECRET` (gitignored `.env.ga4-admin.local`) | `CONSENT_G` = `analytics.edit` only. Never blanket `analytics`. Never merge into `CONSENT_A` | `PLUGIN_DATA/google-oauth-ga4-admin.json` (0600) or `GOOGLE_GA4_ADMIN_ACCESS_TOKEN`. Do not overwrite A | Wave 10 plumbing only — no Admin mutate tools yet. CLI `auth login-ga4-admin` | Missing store/token → `CONSENT_G_REQUIRED`. Login does **not** flip `DGTL_WRITES_ENABLED` |
