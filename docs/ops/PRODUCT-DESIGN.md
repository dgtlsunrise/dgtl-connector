...
| **G** — GA4 Admin writes | **Separate** Desktop client (or later extend W — **never A**). Env: `GOOGLE_OAUTH_GA4_ADMIN_CLIENT_ID` / `SECRET` (gitignored `.env.ga4-admin.local`) | `CONSENT_G` = `analytics.edit` only. Never blanket `analytics`. Never merge into `CONSENT_A` | `PLUGIN_DATA/google-oauth-ga4-admin.json` (0600) or `GOOGLE_GA4_ADMIN_ACCESS_TOKEN`. Do not overwrite A | Wave 11 named Admin tools via **`GoogleGa4AdminHttp`**. CLI `auth login-ga4-admin` | Missing store/token → `CONSENT_G_REQUIRED`. Flag off → `WRITE_NOT_ENABLED`. Login does **not** flip `DGTL_WRITES_ENABLED` |
| **S** — GSC writes | **Separate** Desktop client (or later extend W — **never A**). Env: `GOOGLE_OAUTH_GSC_WRITE_CLIENT_ID` / `SECRET` (gitignored `.env.gsc-write.local`) | `CONSENT_S` = `webmasters` (write, not `.readonly`). Never merge into `CONSENT_A` | `PLUGIN_DATA/google-oauth-gsc-write.json` (0600) or `GOOGLE_GSC_WRITE_ACCESS_TOKEN`. Do not overwrite A | Wave 12 `gsc_submit_sitemap` / `gsc_delete_sitemap` via **`GoogleGscWriteHttp`**. CLI `auth login-gsc-write` | Missing store/token → `CONSENT_S_REQUIRED`. Flag off → `WRITE_NOT_ENABLED`. Login does **not** flip `DGTL_WRITES_ENABLED` |
...
`CONSENT_W` is **never** merged into `CONSENT_A`. Tests in `tests/consent-w.test.ts` lock that invariant. `CONSENT_G` / `CONSENT_S` are likewise never merged into A (`tests/consent-gs.test.ts`: `A ∩ G = ∅`, `A ∩ S = ∅`). **`adwords` is never added to Consent A**, even to “unblock” the gateway (kill criterion 1).
...
Wave 11 shipped named GA4 Admin tools. Wave 12 ships sitemap submit/delete on Consent S. Wave 10 added Consent G / Consent S plumbing (separate stores + login CLIs) so those tools landed **without touching Consent A**. No request-indexing tool.
