...
| **S** — GSC writes | **Separate** Desktop client (or later extend W — **never A**). Env: `GOOGLE_OAUTH_GSC_WRITE_CLIENT_ID` / `SECRET` (gitignored `.env.gsc-write.local`) | `CONSENT_S` = `webmasters` (write, not `.readonly`). Never merge into `CONSENT_A` | `PLUGIN_DATA/google-oauth-gsc-write.json` (0600) or `GOOGLE_GSC_WRITE_ACCESS_TOKEN`. Do not overwrite A | Wave 10 plumbing only — no GSC mutate tools yet. CLI `auth login-gsc-write` | Missing store/token → `CONSENT_S_REQUIRED`. Login does **not** flip `DGTL_WRITES_ENABLED` |
...
GSC writes (sitemap submit) and GA4 Admin (`analytics.edit`) stay **out of tools/list** until a named Wave 11+ tool ships. Wave 10 added Consent G / Consent S plumbing (separate stores + login CLIs) so those tools can land **without touching Consent A**. No request-indexing tool.
