...
Measured in this repo on `src/google/scopes.ts` `CONSENT_A` and `plugin.json` `com.dgtlsunrise.consentA`. Free Connect already requests the full set. The agent does not change Cloud Console.
...
| --- | --- | --- |
| M1 | PKCE URL + consent. Address bar shows the four manage strings plus the readonly trio and identity. App name **DGTL Sunrise**. No Ads. | String match between OAuth request and Console. Consent shows edit/publish. |
...
- [ ] Revoke the old grant. Run `auth login`. Confirm the printed URL `scope=` includes the nine strings above and omits `adwords` / `content` / `business.manage`.
...
- [ ] Paste the nine scope strings and say they are the same strings Console Data Access lists for this Desktop client. Ask them to match the URL `scope=` query, not a host-injected leftover grant.
...
- [ ] Do not change Cloud Console from an agent. If Console Data Access already lists the three write scopes, leave it. The binary already requests them.
- [ ] Do not send a new email thread.

Justifications to reuse: [OAUTH-CONSENT-COPY.md](OAUTH-CONSENT-COPY.md) (`analytics.edit`, `tagmanager.edit.containers`, `tagmanager.publish`, `webmasters`).
