...
| `https://www.googleapis.com/auth/tagmanager.edit.containerversions` | Tag Manager versions | **Staging only** until Google verifies it: Free Connect requests it only when `DGTL_GOOGLE_STAGING_SCOPES=1`. Create a container version before publish (`create_version`). Without it, live `gtm_publish_container` returns `CONSENT_MISSING` before any Google call; `dry_run` still works. Confirm-gated. `dry_run` default. |
