...
- `catalog.json` — 26 Consent A kernel tools (vendored 22 + `ga4_list_account_summaries` + `gsc_describe_schema` + Wave 13 `gtm_list_clients` + `gtm_list_environments`) plus gated families (writes, GBP, Ads/Meta Polar, Shopify local-free, license, support_packet / feedback diagnostics). Shopify is not Polar and is not in the 26 kernel.
