...
1. **Missing / expired / invalid JWT** → buy Pro ($19/mo flat unlimited) at https://buy.polar.sh/polar_cl_aIrywIIxJ2cOwj70VQAcJn2umEgSS9kWBMUJS241Dll (site: https://www.dgtlsunrise.com/). See `pro-upgrade`.
...
| --- | --- | --- |
| `LICENSE_REQUIRED` | No/expired/invalid DGTL license JWT | Free tools still work. Follow the ladder above (buy → redeem → Ads/Meta OAuth → gateway). Pro is $19/mo flat unlimited. CTA: https://buy.polar.sh/polar_cl_aIrywIIxJ2cOwj70VQAcJn2umEgSS9kWBMUJS241Dll (https://www.dgtlsunrise.com/). Prefer `auth redeem`; or paste JWT (`DGTL_LICENSE_JWT` / `PLUGIN_DATA/license.jwt`). Never ask for a developer-token. See `pro-upgrade`. |
...
| `WRITE_NOT_ENABLED` | `DGTL_WRITES_ENABLED` false | Live Merchant Center ProductInput / data-source writes fail closed. Marketplace default off. Free Google GA4 / GTM / GSC and local Shopify / Klaviyo writes ignore this flag — they need Connect plus confirm. See `gtm-readonly-limits`. |
| `CONSENT_W_REQUIRED` | GTM write scopes missing | Run `auth login` (alias `login-write`). Legacy `google-oauth-write.json` still accepted. Live mutate needs confirm with the container publicId. |
| `CONSENT_G_REQUIRED` | GA4 Admin write path but `analytics.edit` missing | Run `auth login` (alias `login-ga4-admin`). Legacy `google-oauth-ga4-admin.json` still accepted. Live mutate needs confirm with the resource id. |
| `CONSENT_S_REQUIRED` | GSC sitemap write path but `webmasters` write missing | Run `auth login` (alias `login-gsc-write`). Legacy `google-oauth-gsc-write.json` still accepted. Live `gsc_submit_sitemap` / `gsc_delete_sitemap` need confirm with the exact `site_url`. |
