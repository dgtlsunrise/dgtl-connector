...
3. For a human ticket, call `support_packet` with the last tool name, `error_code`, and resource id (no tokens). It returns plugin version, host, dual-gate flag booleans (plugin Ads/Meta/TikTok mutate + CAPI/Events/Ads Data Manager default **on**; sGTM ingest test / writes / GBP default **off**; Worker fail-closed; live hop needs both), gateway configured/reachable, license feature names (Polar `sgtm` reserved, default-off, not minted), consent-store presence (A/C/W/G/S/MC/B + Meta/TikTok/Shopify/Klaviyo), conversion-fabric `apply_key_present` (boolean only), and a docs-relative `runbook` + `next_human_step` (`docs/ops/RUNBOOKS.md`) when the code maps. Use those intake fields; never ask for tokens or apply keys. GBP: flag off → `GBP_NOT_ENABLED`; flag on without Consent B → `GBP_NOT_CONNECTED`. Never put `business.manage` or write scopes on Consent A. Consent G/S missing → `CONSENT_G_REQUIRED` / `CONSENT_S_REQUIRED`. Klaviyo missing `pk_` → `KLAVIYO_NOT_CONNECTED`. Error codes stay stable — do not invent `gads_mutate`.
4. Branch:
