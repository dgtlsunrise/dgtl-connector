...
`GET /v1/connect` returns `{shopify, klaviyo}` as `linked` or `null`. `POST /v1/connect/shopify` body `{shop, access_token}` seals the Admin API token and returns `{connected: true, shop}`. `POST /v1/connect/klaviyo` body `{api_key}` seals a `pk_` private key and returns `{connected: true}`. `DELETE` on either path clears that link and returns `{connected: false}`. These routes do not call Shopify or Klaviyo, and they do not return the token, the key, or ciphertext. Shopify writes are not in this API. Klaviyo writes are not in this API.
